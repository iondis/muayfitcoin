...
#if defined(NDEBUG)
#error "Mfit cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 6.51605 * COIN;
    }   else if (nHeight <= 1100000 ) {
        nSubsidy = 6.1902475 * COIN;
    }   else if (nHeight <= 1200000 ) {
        nSubsidy = 5.880735 * COIN;
    }   else {
        nSubsidy = 5.5867 * COIN;
    }   
...
bool static FlushStateToDisk(CValidationState& state, FlushStateMode mode)
{
    ...
    try {
        if ((mode == FLUSH_STATE_ALWAYS) ||
            ...
            (mode == FLUSH_STATE_PERIODIC && GetTimeMicros() > nLastWrite + DATABASE_WRITE_INTERVAL * 100000)) {
...
static CBlockIndex* FindMostWorkChain()
{
    do {
        ...
        while (pindexTest && !chainActive.Contains(pindexTest)) {
            ...
            // for the most work chain if we come mfitoss them; we can't switch
...
bool CVerifyDB::VerifyDB(CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        nCheckDepth = 100000000; // suffices until the year 19000
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        BOOST_FOREACH (CAddress& addr, vAddr) {
            ...
            if (addr.nTime <= 10000000 || addr.nTime > nNow + 10 * 60)
    ...
    // getaddr message mitigates the attack.
        ...
        obfuScationPool.ProcessMessagePrivateSend(pfrom, strCommand, vRecv);
...
{
    {
        ...
        if (pto->nPingNonceSent == 0 && pto->nPingUsecStart + PING_INTERVAL * 100000 < GetTimeMicros()) {
        ...
        if (!pto->fDisconnect && state.nStallingSince && state.nStallingSince < nNow - 100000 * BLOCK_STALLING_TIMEOUT) {
