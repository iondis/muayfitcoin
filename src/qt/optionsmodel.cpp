...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MFIT);
    ...
    if (!settings.contains("nAnonymizeMfitAmount"))
        settings.setValue("nAnonymizeMfitAmount", 1000);

    nPrivateSendRounds = settings.value("nPrivateSendRounds").toLongLong();
    nAnonymizeMfitAmount = settings.value("nAnonymizeMfitAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMfitAmount"))
        SoftSetArg("-anonymizemfitamount", settings.value("nAnonymizeMfitAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mfit.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMfitAmount:
            return QVariant(nAnonymizeMfitAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case PrivateSendRounds:
            ...
            break;
        case AnonymizeMfitAmount:
            nAnonymizeMfitAmount = value.toInt();
            settings.setValue("nAnonymizeMfitAmount", nAnonymizeMfitAmount);
            emit anonymizeMfitAmountChanged(nAnonymizeMfitAmount);
            break;
        case CoinControlFeatures:
