...
void CPrivateSendPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        LogPrintf("CPrivateSendPool::Check() -- IS MASTER -- TRANSMITTING OBFUSCATION\n");
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Mfit and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 MFIT per transaction on average.
            */
            if (r <= 10) {
...
bool CPrivateSendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeMfitAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CPrivateSendPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100MFIT+1 ( bit on if present )
    // bit 1 - 10MFIT+1
    // bit 2 - 1MFIT+1
    // bit 3 - .1MFIT+1
    // bit 3 - non-denom
