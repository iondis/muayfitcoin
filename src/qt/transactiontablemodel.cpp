...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::MFIT)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("MFIT")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    QString strAnonymizeMfitAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeMfitAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeMfitAmount = strAnonymizeMfitAmount.remove(strAnonymizeMfitAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMfitAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeMfitAmount * COIN) nMaxToAnonymize = nAnonymizeMfitAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeMfitAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeMfitAmount));
        strAnonymizeMfitAmount = strAnonymizeMfitAmount.remove(strAnonymizeMfitAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMfitAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    } else {
